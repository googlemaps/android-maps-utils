    ...
    private inner class RenderTask(
        ...
        fun setMapZoom(zoom: Float) {
            ...
            sphericalMercatorProjection =
                SphericalMercatorProjection(256.0 * 2.0.pow(mapZoom.coerceAtMost(this@DefaultClusterRenderer.zoom).toDouble()))
        }

        override fun run() {
            if (!shouldRender(immutableOf(this@DefaultClusterRenderer.clusters), immutableOf(clusters))) {
                callback?.run()
                return
            }
            ...
            val zoom = mapZoom
            val zoomingIn = zoom > this@DefaultClusterRenderer.zoom
            val zoomDelta = zoom - this@DefaultClusterRenderer.zoom

            val markersToRemove = this@DefaultClusterRenderer.markers
            ...
            if (this@DefaultClusterRenderer.clusters != null && animate) {
                existingClustersOnScreen = mutableListOf()
                for (c in this@DefaultClusterRenderer.clusters!!) {
