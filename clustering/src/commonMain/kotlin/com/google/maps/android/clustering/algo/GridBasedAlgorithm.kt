...
class GridBasedAlgorithm<T : ClusterItem> : AbstractAlgorithm<T>() {
    ...
    private val mItemsLock = PlatformLock()
    private val mItems: MutableSet<T> = HashSet()

    override fun addItem(item: T): Boolean = mItemsLock.withLock { mItems.add(item) }

    override fun addItems(items: Collection<T>): Boolean = mItemsLock.withLock { mItems.addAll(items) }

    override fun clearItems() {
        mItemsLock.withLock { mItems.clear() }
    }

    override fun removeItem(item: T): Boolean = mItemsLock.withLock { mItems.remove(item) }

    override fun removeItems(items: Collection<T>): Boolean = mItemsLock.withLock { mItems.removeAll(items.toSet()) }
    ...
    override val items: Collection<T>
        get() = mItemsLock.withLock { mItems.toSet() }
