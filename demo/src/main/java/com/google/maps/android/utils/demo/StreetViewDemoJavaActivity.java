 ...
 *
 * <p>This activity performs the following actions: 1. Sets up the layout to fit system windows. 2.
 * Checks if a valid Google Maps API key is present. 3. Fetches Street View data for two predefined
 * locations using asynchronous callbacks. 4. Displays the results of the Street View data fetch on
 * the screen.
 */
public class StreetViewDemoJavaActivity extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    StreetViewJavaHelper.fetchStreetViewData(
        ...
        new StreetViewJavaHelper.StreetViewCallback() {
          @Override
          public void onStreetViewResult(@NonNull Status status) {
            // Updates the UI with the result on the UI thread.
            runOnUiThread(
                () ->
                    ((TextView) findViewById(R.id.textViewFirstLocation))
                        .setText("Location 1 is supported in StreetView: " + status));
          }
          ...
          public void onStreetViewError(@NonNull Exception e) {
            ...
            Toast.makeText(
                    StreetViewDemoJavaActivity.this,
                    "Error fetching Street View data: " + e.getMessage(),
                    Toast.LENGTH_SHORT)
                .show();
          }
        });
    ...
    StreetViewJavaHelper.fetchStreetViewData(
        ...
        new StreetViewJavaHelper.StreetViewCallback() {
          ...
          public void onStreetViewResult(@NonNull Status status) {
            ...
            runOnUiThread(
                () ->
                    ((TextView) findViewById(R.id.textViewSecondLocation))
                        .setText("Location 2 is  supported in StreetView: " + status));
          }
