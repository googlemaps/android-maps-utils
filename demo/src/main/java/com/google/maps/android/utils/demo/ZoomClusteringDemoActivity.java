 ...
 */
public class ZoomClusteringDemoActivity extends BaseDemoActivity
  ...
  public boolean onClusterClick(Cluster<MyItem> cluster) {
    ...
    Toast.makeText(this, cluster.getSize() + " (including " + title + ")", Toast.LENGTH_SHORT)
        .show();
  ...
  protected void startDemo(boolean isRestore) {
    if (!isRestore) {
      getMap()
          .moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(18.528146, 73.797726), 9.5f));
    }
    ...
    String snippet =
        "This item wouldn't have changed to a marker if we didn't override shouldRenderAsCluster() AND shouldRender()";
  ...
  private class ZoomBasedRenderer extends DefaultClusterRenderer<MyItem>
      implements GoogleMap.OnCameraIdleListener {
    private Float zoom = 15f;
    private Float oldZoom;
    private static final float ZOOM_THRESHOLD = 12f;

    public ZoomBasedRenderer(
        Context context, GoogleMap map, ClusterManager<MyItem> clusterManager) {
      super(context, map, clusterManager);
    }

    /**
     * The {@link ClusterManager} will call the {@link this.onCameraIdle()} implementation of any
     * Renderer that implements {@link GoogleMap.OnCameraIdleListener} <i>before</i> clustering and
     * rendering takes place. This allows us to capture metrics that may be useful for clustering,
    ...
    /**
     * You can override this method to control when the cluster manager renders a group of items as
     * a cluster (vs. as a set of individual markers).
     *
     * <p>In this case, we want single markers to show up as a cluster when zoomed out, but
     ...
     *
     * <p>However, in our case we need to change this behavior. As defined in {@link
     * this.shouldRenderAsCluster()}, we want an item to render as a cluster above a certain zoom
     * level and as a marker below a certain zoom level <i>even if the contents of the clusters
     * themselves did not change</i>. In this case, we need to override this method to implement
     * this new optimization behavior.
     *
     * <p>Note that always returning true from this method could potentially have negative
     ...
     * @return true if the new clusters should be rendered on the map, and false if they should not.
     */
    @Override
    protected boolean shouldRender(
        @NonNull Set<? extends Cluster<MyItem>> oldClusters,
        @NonNull Set<? extends Cluster<MyItem>> newClusters) {
      if (crossedZoomThreshold(oldZoom, zoom)) {
        // Render when the zoom level crosses the threshold, even if the clusters don't change
        return true;
      } else {
        // If clusters didn't change, skip render for optimization using default super
        // implementation
    ...
    /**
     * Returns true if the transition between the two zoom levels crossed a defined threshold, false
     * if it did not.
     *
     ...
     * @return true if the transition between the two zoom levels crossed a defined threshold, false
     *     if it did not.
     */
