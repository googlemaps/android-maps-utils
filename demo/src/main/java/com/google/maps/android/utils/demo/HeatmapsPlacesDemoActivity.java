...
/**
 * A demo of the heatmaps library incorporating radar search from the Google Places API. This
 * demonstrates the usefulness of heatmaps for displaying the distribution of points, as well as
 * demonstrating the various color options and dealing with multiple heatmaps.
 */
public class HeatmapsPlacesDemoActivity extends BaseDemoActivity {
  ...
  /**
   * The search radius which roughly corresponds to the radius of the results from the radar search
   * in meters.
   */
  ...
  /**
   * Takes the input from the user and generates the required heatmap. Called when a search query is
   * submitted
   */
  public void submit(View view) {
    ...
    if (mOverlays.contains(keyword)) {
      ...
      Toast.makeText(
              this, "You can only input " + MAX_CHECKBOXES + " keywords. :(", Toast.LENGTH_SHORT)
          .show();
  ...
  /**
   * Makes four radar search requests for the given keyword, then parses the json output and returns
   * the search results as a collection of LatLng objects.
   *
   * @param keyword A string to use as a search term for the radar search
   * @return Returns the search results from radar search as a collection of LatLng objects, or null
      ...
      try {
        ...
        for (int i = 0; i < pointsJsonArray.length(); i++) {
          if (!results.containsKey(pointsJsonArray.getJSONObject(i).getString("place_id"))) {
            ...
            results.put(
                pointsJsonArray.getJSONObject(i).getString("place_id"),
  ...
  /**
   * Async task, because finding the points cannot be done on the main thread, while adding the
   * overlay must be done on the main thread.
   */
  private class MakeOverlayTask extends AsyncTask<String, Integer, PointsKeywords> {
    ...
    protected void onPostExecute(PointsKeywords pointsKeywords) {
      ...
      if (!points.isEmpty()) {
        if (mOverlays.size() < MAX_CHECKBOXES) {
          ...
          HeatmapTileProvider provider =
              new HeatmapTileProvider.Builder()
                  ...
                  .build();
          TileOverlay overlay =
              getMap().addTileOverlay(new TileOverlayOptions().tileProvider(provider));
        ...
        progressBar.setVisibility(View.GONE);
        Toast.makeText(
                HeatmapsPlacesDemoActivity.this, "No results for this query :(", Toast.LENGTH_SHORT)
            .show();
      }
    }
  }
