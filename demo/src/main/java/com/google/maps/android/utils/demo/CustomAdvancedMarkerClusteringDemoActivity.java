...
public class CustomAdvancedMarkerClusteringDemoActivity extends BaseDemoActivity
  ...
  private class AdvancedMarkerRenderer extends DefaultAdvancedMarkersClusterRenderer<Person> {
    ...
    protected void onBeforeClusterItemRendered(
        @NonNull Person person, @NonNull AdvancedMarkerOptions markerOptions) {
      markerOptions
    ...
    private PinConfig.Builder getPinConfig(Person person) {
      ...
      pinConfigBuilder.setGlyph(
          new PinConfig.Glyph(BitmapDescriptorFactory.fromResource(person.profilePhoto)));
    ...
    protected void onBeforeClusterRendered(
        @NonNull Cluster<Person> cluster, @NonNull AdvancedMarkerOptions markerOptions) {
  ...
  protected void startDemo(boolean isRestore) {
    if (!isRestore) {
      getMap()
          .moveCamera(CameraUpdateFactory.newLatLngZoom(new LatLng(51.503186, -0.126446), 9.5f));
    }

    // This line is extremely important to initialise the advanced markers. Without it, advanced
    // markers will not work.
