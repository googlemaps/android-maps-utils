...
public class ClusteringDiffDemoActivity extends BaseDemoActivity
  ...
  public boolean onClusterClick(Cluster<Person> cluster) {
    ...
    Toast.makeText(
            this,
            getString(R.string.cluster_info_fmt, cluster.getSize(), firstName),
            Toast.LENGTH_SHORT)
        .show();
  ...
  /**
   * Draws profile photos inside markers (using IconGenerator). When there are multiple people in
   * the cluster, draw multiple photos (using MultiDrawable).
   */
  ...
  private class PersonRenderer extends ClusterRendererMultipleItems<Person> {
    ...
    protected void onBeforeClusterItemRendered(
        @NonNull Person person, @NonNull MarkerOptions markerOptions) {
    ...
    /**
     * Get a descriptor for a single person (i.e., a marker outside a cluster) from their profile
    ...
    protected void onBeforeClusterRendered(
        @NonNull Cluster<Person> cluster, @NonNull MarkerOptions markerOptions) {
      // Draw multiple people.
      // Note: this method runs on the UI thread. Don't spend too much time in here (like in this
      // example).
    ...
    private BitmapDescriptor getClusterIcon(Cluster<Person> cluster) {
      ...
      for (Person p : cluster.getItems()) {
        ...
        Drawable drawable =
            ResourcesCompat.getDrawable(getBaseContext().getResources(), p.profilePhoto, null);
        if (drawable != null) {
