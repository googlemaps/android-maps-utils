...
class Gradient
    ...
    constructor(
        ...
        private fun generateColorIntervals(): HashMap<Int, ColorInterval> {
            ...
            if (startPoints[0] != 0f) {
                val initialColor =
                    ColorUtils.argb(
                        0,
                        ColorUtils.red(colors[0]),
                        ColorUtils.green(colors[0]),
                        ColorUtils.blue(colors[0]),
                    )
         ...
         * multiplied by this factor. The default value is [HeatmapConstants.DEFAULT_OPACITY].
         * @return An integer array representing the color map, where each element is a color integer.
         */
        @JvmOverloads
        fun generateColorMap(opacity: Double = HeatmapConstants.DEFAULT_OPACITY): IntArray {
            ...
            if (opacity != 1.0) {
                for (i in 0 until colorMapSize) {
                    ...
                    colorMap[i] =
                        ColorUtils.argb(
                            (ColorUtils.alpha(c) * opacity).toInt(),
                            ColorUtils.red(c),
                            ColorUtils.green(c),
                            ColorUtils.blue(c),
                        )
        ...
        companion object {
            ...
            fun interpolateColor(
                ...
                val alpha = ((ColorUtils.alpha(color2) - ColorUtils.alpha(color1)) * ratio + ColorUtils.alpha(color1)).toInt()
                val hsv1 = FloatArray(3)
                ColorUtils.rgbToHsv(ColorUtils.red(color1), ColorUtils.green(color1), ColorUtils.blue(color1), hsv1)
                val hsv2 = FloatArray(3)
                ColorUtils.rgbToHsv(ColorUtils.red(color2), ColorUtils.green(color2), ColorUtils.blue(color2), hsv2)
                ...
                return ColorUtils.hsvToColor(alpha, result)
            }
        }
    }
