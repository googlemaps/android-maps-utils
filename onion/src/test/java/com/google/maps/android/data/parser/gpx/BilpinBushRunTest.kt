...
class BilpinBushRunTest {

    @Test
    fun `test parse Bilpin Bush Run GPX`() {
        val stream = javaClass.classLoader!!.getResourceAsStream("Bilpin_Bush_Run.gpx")
    ...
    @Test
    fun `test map Bilpin Bush Run GPX to Layer`() {
        val stream = javaClass.classLoader!!.getResourceAsStream("Bilpin_Bush_Run.gpx")
        val parser = GpxParser()
        val gpx = parser.parse(stream)
