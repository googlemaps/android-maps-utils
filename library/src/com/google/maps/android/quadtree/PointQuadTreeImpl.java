...
public class PointQuadTreeImpl<T extends QuadTree.Item> implements QuadTree<T> {
     ...
     * Child quads.
     */
    private PointQuadTreeImpl<T>[] mChildren = null;
     ...
     * @param maxY
     */
    public PointQuadTreeImpl(double minX, double maxX, double minY, double maxY) {
        this(new Bounds(minX, maxX, minY, maxY));
    }

    public PointQuadTreeImpl(Bounds bounds) {
        this(bounds, 0);
    }

    private PointQuadTreeImpl(double minX, double maxX, double minY, double maxY, int depth) {
        this(new Bounds(minX, maxX, minY, maxY), depth);
    }

    private PointQuadTreeImpl(Bounds bounds, int depth) {
        mBounds = bounds;
        mDepth = depth;
    }
    ...
    private void search(Bounds searchBounds, Collection<T> results) {
        ...
        if (this.mChildren != null) {
            for (PointQuadTreeImpl<T> quad : mChildren) {
