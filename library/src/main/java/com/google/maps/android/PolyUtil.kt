...
object PolyUtil {
    ...
    const val DEFAULT_TOLERANCE: Double = 0.1 // meters.
    ...
    fun locationIndexOnEdgeOrPath(
        ...
        val tolerance: Double = toleranceEarth / EARTH_RADIUS
        ...
        if (geodesic) {
            for (point2 in poly) {
                ...
                if (isOnSegmentGC(lat1, lng1, lat2, lng2, lat3, lng3, havTolerance)) {
                    return max(0.0, (idx - 1).toDouble()).toInt()
                }
                lat1 = lat2
                lng1 = lng2
                idx++
            ...
            for (point2 in poly) {
                ...
                if (max(lat1, lat2) >= minAcceptable && min(lat1, lat2) <= maxAcceptable) {
                    ...
                    for (x3 in xTry) {
                        ...
                        val t = if (len2 <= 0) 0.0 else clamp(
                            (x3 * x2 + (y3 - y1) * dy) / len2,
                            0.0,
                            1.0
                        )
...
private fun Long.encodeTo(stringBuilder: StringBuilder) {
    var value = if (this < 0) {
        (this shl 1).inv()
    } else {
        this shl 1
    ...
    while (value >= 0x20) {
        stringBuilder.append(Character.toChars(((0x20L or (value and 0x1fL)) + 63).toInt()))
        value = value shr 5
    }

    stringBuilder.append(Character.toChars((value + 63).toInt()))
}
