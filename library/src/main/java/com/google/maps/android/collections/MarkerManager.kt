...
open class MarkerManager(map: GoogleMap) :
    ...
    open inner class Collection : MapObjectManager<Marker, Collection>.Collection() {
        ...
        open fun addMarker(opts: MarkerOptions): Marker =
            checkNotNull(mMap.addMarker(opts)) { "Failed to add Marker to GoogleMap" }
                .also { super.add(it) }

        open fun addMarker(opts: AdvancedMarkerOptions): Marker =
            checkNotNull(mMap.addMarker(opts)) { "Failed to add AdvancedMarker to GoogleMap" }
                .also { super.add(it) }
        ...
        open fun addAll(opts: KotlinCollection<MarkerOptions>, defaultVisible: Boolean) {
            for (opt in opts) {
                addMarker(opt).isVisible = defaultVisible
            }
        }
        ...
        override fun remove(marker: Marker?): Boolean = super.remove(marker)
