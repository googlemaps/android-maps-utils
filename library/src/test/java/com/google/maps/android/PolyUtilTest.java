...
/**
 * This class defines a series of tests for the {@link PolyUtil} class. Each test is designed to
 * verify the correctness of a specific geometric utility function provided by {@link PolyUtil},
 * such as checking if a point is contained within a polygon, if it lies on an edge, or simplifying
 * a polyline.
 *
 * <p>The tests are structured to cover a wide range of scenarios, including edge cases like empty
...
public class PolyUtilTest {
  ...
  /**
   * A helper method to construct a {@link List} of {@link LatLng} objects from a series of latitude
  ...
  /**
   * A helper method to test the {@link PolyUtil#containsLocation(LatLng, List, boolean)} method. It
   * asserts that all points in the {@code yes} list are contained within the polygon, and all
   * points in the {@code no} list are not. This is tested for both geodesic and rhumb line paths.
   *
  ...
  /**
   * A helper method to test the {@link PolyUtil#isLocationOnEdge(LatLng, List, boolean)} and {@link
   * PolyUtil#isLocationOnPath(LatLng, List, boolean)} methods. It asserts that all points in the
   * {@code yes} list are on the edge of the polygon, and all points in the {@code no} list are not.
   *
  ...
  /**
   * A helper method to test the {@link PolyUtil#locationIndexOnPath(LatLng, List, boolean)}. It
   * asserts that the returned index for a given point on a polyline is as expected.
   *
   * @param geodesic Whether to use geodesic or rhumb line paths.
  ...
  /**
   * Overloaded helper method for {@link #locationIndexCase(boolean, List, LatLng, int)} that tests
   * for both geodesic and rhumb line paths.
   */
  ...
  /**
   * Overloaded helper method for {@link #locationIndexToleranceCase(boolean, List, LatLng, int)}
   * that tests for both geodesic and rhumb line paths.
   */
  ...
  public void testOnEdge() {
    ...
    onEdgeCase(
        ...
        makeList(
            20, 10, 10 - big, 5 - big / 2, 30 + 2 * big, 15 + big, 10 + 2 * big, 5, 10, 5 + big));
  ...
  public void testLocationIndexTolerance() {
    ...
    // Test tolerance.
    locationIndexToleranceCase(makeList(0, 90 - small, 0, 90, 0, 90 + small), new LatLng(0, 90), 0);
  ...
  /**
   * The following method checks whether {@link PolyUtil#distanceToLine(LatLng, LatLng, LatLng)
   * distanceToLine()} } is determining the distance between a point and a segment accurately.
   *
   * <p>Currently there are tests for different orders of magnitude (i.e., 1X, 10X, 100X, 1000X), as
   * well as a test where the segment and the point lie in different hemispheres.
   *
   * <p>If further tests need to be added here, make sure that the distance has been verified with
