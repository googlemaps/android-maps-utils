...
public class SphericalUtilTest {
  ...
  public void testDistances() {
    assertThat(SphericalUtil.computeDistanceBetween(up, down))
        .isWithin(1e-6)
        .of(Math.PI * EARTH_RADIUS);
  }
  ...
  public void testComputeOffsetOrigin() {
    expectLatLngApproxEquals(
        front, Objects.requireNonNull(SphericalUtil.computeOffsetOrigin(front, 0, 0)));

    expectLatLngApproxEquals(
        front,
        Objects.requireNonNull(
            SphericalUtil.computeOffsetOrigin(new LatLng(0, 45), Math.PI * EARTH_RADIUS / 4, 90)));
    expectLatLngApproxEquals(
        front,
        Objects.requireNonNull(
    ...
    expectLatLngApproxEquals(
        front,
        Objects.requireNonNull(
            SphericalUtil.computeOffsetOrigin(new LatLng(45, 0), Math.PI * EARTH_RADIUS / 4, 0)));
    expectLatLngApproxEquals(
        front,
        Objects.requireNonNull(
            SphericalUtil.computeOffsetOrigin(
    ...
    // First 'over' the pole.
    assertThat(
            SphericalUtil.computeOffsetOrigin(new LatLng(80, 0), Math.PI * EARTH_RADIUS / 4, 180))
        .isNull();
    // Second a distance that doesn't fit on the earth.
    assertThat(SphericalUtil.computeOffsetOrigin(new LatLng(80, 0), Math.PI * EARTH_RADIUS / 4, 90))
        .isNull();
  }
  ...
  public void testComputeOffsetAndBackToOrigin() {
    ...
    expectLatLngApproxEquals(
        start, Objects.requireNonNull(SphericalUtil.computeOffsetOrigin(end, distance, heading)));
    ...
    expectLatLngApproxEquals(
        new LatLng(0, 90), SphericalUtil.computeOffset(start, Math.PI * EARTH_RADIUS / 2, 90));
    ...
    Assert.assertNotNull(start);
    expectLatLngApproxEquals(
        new LatLng(90, 0), SphericalUtil.computeOffset(start, Math.PI * EARTH_RADIUS / 4, 0));
  }
  ...
  public void testInterpolate() {
    ...
    expectLatLngApproxEquals(
        ...
        SphericalUtil.interpolate(
            new LatLng(-37.756891, 175.325262), new LatLng(-37.756853, 175.325242), 0.5));
  }
  ...
  public void testComputeLength() {
    ...
    assertThat(SphericalUtil.computeLength(latLngs))
        .isWithin(1)
        .of(Math.toRadians(0.1) * Math.sqrt(2) * EARTH_RADIUS);
  ...
  public void testComputeSignedTriangleArea() {
    assertThat(
            computeSignedTriangleArea(new LatLng(0, 0), new LatLng(0, 0.1), new LatLng(0.1, 0.1)))
        .isWithin(1e-6)
        .of(Math.toRadians(0.1) * Math.toRadians(0.1) / 2);
  ...
  public void testComputeArea() {
    assertThat(SphericalUtil.computeArea(Arrays.asList(right, up, front, down, right)))
        .isWithin(.4)
        .of(Math.PI * EARTH_RADIUS * EARTH_RADIUS);

    assertThat(SphericalUtil.computeArea(Arrays.asList(right, down, front, up, right)))
        .isWithin(.4)
        .of(Math.PI * EARTH_RADIUS * EARTH_RADIUS);
  }
  ...
  public void testComputeSignedArea() {
    ...
    assertThat(-SphericalUtil.computeSignedArea(path))
        .isWithin(0)
        .of(SphericalUtil.computeSignedArea(pathReversed));
  }
  ...
  public void testGetPointOnPolyline() {
    ...
    expectLatLngApproxEquals(
        midAB, SphericalUtil.getPointOnPolyline(polyline, (abLength / 2) / totalLength));

    // Test for a point on a vertex
    final double aToCLength = SphericalUtil.computeLength(Arrays.asList(a, b, c));
    expectLatLngApproxEquals(
        c, SphericalUtil.getPointOnPolyline(polyline, aToCLength / totalLength));
  }
  ...
  public void testGetPolylinePrefix() {
    ...
    final double totalLength = SphericalUtil.computeLength(polyline);
    final double abLength = SphericalUtil.computeDistanceBetween(a, b);
    List<LatLng> prefixMidAB =
        SphericalUtil.getPolylinePrefix(polyline, (abLength / 2) / totalLength);
